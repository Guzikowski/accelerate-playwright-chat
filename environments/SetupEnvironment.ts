...
export class SetupEnvironment {
    public static initialise(configName?: string, requireOverride: boolean = false) {
        ...
        if (requireOverride) {
            ...
            var result = dotenv.config({ path: `./src/environments/.env.${process.env.CONFIG_ENV_NAME}` })
            if (result.error) {
    ...
    private static overrideConfiguration(): void {
        const envConfig = dotenv.parse(fs.readFileSync(`./src/environments/.env.${process.env.CONFIG_ENV_NAME}`))
