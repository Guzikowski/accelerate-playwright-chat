...
export class ChatSecurity {
	...
	private locatorDismissBanner = '[aria-label="Dismiss"]';
	...
	public async Login(page: Page, userName: string, password: string) {
		...
		await expect(page).toHaveURL(this.alreadyLoggedInUrl);
		let isPresent = await page.isVisible(this.locatorDismissBanner);
		if (isPresent) {
			await page.click(this.locatorDismissBanner);
		}
	}
